...
st.title2('반도체 주식 데이터 Dashboard')
...
reversed_ticker2 = dict(map(reversed,tickers.items()))

dropdown2 = st.multiselect('select',tickers.keys())

start2 = st.date_input('Start', value=pd.to_datetime('2019-01-01'))

end2 = st.date_input('End',value=pd.to_datetime('today'))


if len(dropdown2) > 0 :
    for i in dropdown2:
        df2 = yf.download2(tickers[i],start,end)['Adj Close']
        st.title2(reversed_ticker[tickers[i]])
        st.line_chart2(df)